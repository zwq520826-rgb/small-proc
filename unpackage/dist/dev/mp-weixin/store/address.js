...
async function loadFromCloud() {
  try {
    ...
    if (res && res.code === 0 && Array.isArray(res.data)) {
      ...
      state.selectedAddress = state.addressList.find((item) => item.isDefault) || state.addressList[0] || null;
      save();
      return true;
    }
  } catch (e) {
    common_vendor.index.__f__("warn", "at store/address.js:34", "load address from cloud failed", e);
  }
  return false;
}
async function ensureInit() {
  ...
  try {
    ...
    common_vendor.index.__f__("warn", "at store/address.js:59", "load address failed", e);
  }
}
function save() {
  try {
    ...
    common_vendor.index.__f__("warn", "at store/address.js:67", "save address failed", e);
  }
}
function useAddressStore() {
  ...
  const setSelected = async (id) => {
    ...
    try {
      ...
      common_vendor.index.__f__("warn", "at store/address.js:92", "set default address failed", e);
  ...
  const addAddress = async (payload) => {
    try {
      ...
      common_vendor.index.__f__("warn", "at store/address.js:107", "add address failed", e);
  ...
  const updateAddress = async (id, payload) => {
    ...
    try {
      ...
      common_vendor.index.__f__("warn", "at store/address.js:125", "update address failed", e);
      common_vendor.index.showToast({ title: "保存失败", icon: "none" });
      return false;
    }
  };
  const removeAddress = async (id) => {
    ...
    try {
      ...
      common_vendor.index.__f__("warn", "at store/address.js:143", "delete address failed", e);

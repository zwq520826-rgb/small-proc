...
function sanitizeForLog(input) {
  ...
  if (typeof input === 'string') return input.length > 400 ? `${input.slice(0, 200)}...[truncated]` : input
  if (typeof input !== 'object') return input
  const keys = ['mobile', 'phone', 'idcard', 'id_card', 'password', 'token', 'authorization', 'signature', 'ciphertext', 'nonce', 'api_v3_key', 'appsecret', 'private_key', 'body']
  if (Array.isArray(input)) return input.slice(0, 20).map((x) => sanitizeForLog(x))
  const out = {}
  for (const key of Object.keys(input)) {
    const lowered = key.toLowerCase()
    const sensitive = keys.some((k) => lowered.includes(k))
    const val = input[key]
    out[key] = sensitive
      ? (String(val || '').length <= 4 ? '****' : `${String(val).slice(0, 2)}****${String(val).slice(-2)}`)
      : sanitizeForLog(val)
  }
  return out
}
...
function createLogger({ service = 'unknown', api = '', uid = '', ip = '', requestId = '' } = {}) {
  ...
  return {
    info(payload = {}) { console.log(JSON.stringify({ time: new Date().toISOString(), level: 'info', ...base, ...sanitizeForLog(payload) })) },
    warn(payload = {}) { console.warn(JSON.stringify({ time: new Date().toISOString(), level: 'warn', ...base, ...sanitizeForLog(payload) })) },
    error(payload = {}) { console.error(JSON.stringify({ time: new Date().toISOString(), level: 'error', ...base, ...sanitizeForLog(payload) })) }
  }
}
...
async function checkRateLimit({ service, api, ip = '', uid = '', ipRule, uidRule }) {
  ...
  const run = async (dimension, identity, rule) => {
    ...
    hits.push(now)
    await coll.doc(key).set({ dimension, service, api, identity, window_sec: rule.windowSec, limit: rule.limit, hits, update_time: now, expire_at: now + rule.windowSec * 2000 })
    return { allowed: true }
  }
...
function rateLimitedError({ requestId, retryAfterSec = 1 }) {
  return { code: 'RATE_LIMITED', errCode: 'RATE_LIMITED', message: '请求过于频繁，请稍后重试', statusCode: 429, retryAfter: retryAfterSec, requestId }
}
...
module.exports = { resolveRequestId, hashIp, sanitizeForLog, createLogger, checkRateLimit, rateLimitedError, getRuntimeFlags, withRequestId }
